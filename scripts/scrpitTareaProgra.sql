...
-- ========================================================
-- Tabla: user  (primero, por FKs de otras tablas)
-- ========================================================
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `is_coach` tinyint(1) NOT NULL DEFAULT 0,           -- NUEVO
  `role` varchar(50) DEFAULT NULL,
  `assigned_coach_id` bigint(20) DEFAULT NULL,        -- NUEVO
  PRIMARY KEY (`id`),
  UNIQUE KEY `email` (`email`),
  UNIQUE KEY `uq_user_email` (`email`),
  KEY `idx_user_name_id` (`name`,`id`),
  KEY `idx_user_assigned_coach` (`assigned_coach_id`), -- NUEVO
  CONSTRAINT `fk_user_assigned_coach` FOREIGN KEY (`assigned_coach_id`) REFERENCES `user` (`id`) ON DELETE SET NULL, -- NUEVO
  CONSTRAINT `chk_user_auditor_xor_coach` CHECK (NOT (`is_auditor` = 1 AND `is_coach` = 1)) -- NUEVO
