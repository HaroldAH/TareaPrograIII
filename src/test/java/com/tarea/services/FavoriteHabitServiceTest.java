...
class FavoriteHabitServiceTest {
    ...
    void save_ok() {
        ...
        try (MockedStatic<SecurityUtils> mocked = mockStatic(SecurityUtils.class)) {
            ...
            mocked.when(() -> SecurityUtils.requireSelfOrMutate(1L, Module.HABITS)).thenAnswer(inv -> null);
    ...
    void getFavoriteHabitsListByUser_mapsListDTO() {
        ...
        try (MockedStatic<SecurityUtils> mocked = mockStatic(SecurityUtils.class)) {
            mocked.when(() -> SecurityUtils.requireSelfOrView(1L, Module.HABITS)).thenAnswer(inv -> null);

            List<HabitActivityListDTO> list = service.getFavoriteHabitsListByUser(1L);

            assertEquals(1, list.size());
            assertEquals("Dormir", list.get(0).getName());
            assertEquals("SLEEP", list.get(0).getCategory());
        }
    }
    ...
    void getFavoriteHabitDetailByName_buildsDetailDTO() {
        ...
        var fav = new FavoriteHabit(); var u = new User(); u.setId(1L); fav.setUser(u); fav.setHabit(h);
        ...
        try (MockedStatic<SecurityUtils> mocked = mockStatic(SecurityUtils.class)) {
            ...
            assertEquals("Meditar", dto.getName());
            assertEquals("MENTAL", dto.getCategory());
        }
    }
}
